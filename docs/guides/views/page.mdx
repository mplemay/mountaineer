...
<Note>
    The recommended API is now `Page` with loader functions via `@page.data` and actions via `@page.action`.
    The `ControllerBase` examples below are legacy and will be updated.
</Note>
