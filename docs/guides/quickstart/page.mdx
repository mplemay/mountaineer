...
<Note>
    Mountaineer now supports the route-style `Page` API with `@page.data` and `@page.action`.
    The `ControllerBase` + `@sideeffect`/`@passthrough` examples below are legacy and will be updated.
</Note>
