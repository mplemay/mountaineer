...
<Note>
    The recommended API is now `Page` with `@page.action(update=...)`, which returns a reload instruction.
    The `@sideeffect` and `@passthrough` decorators are deprecated shims and will be removed in a future release.
</Note>
